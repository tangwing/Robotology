...
\[ BORNE\_RATE[i]=(0.0001/180*PI)/(QUAconfig[i].maxTheta - QUAconfig[i].minTheta); \]

Notre motivation de le faire, c'est pour avoir une précision de 0.0001\degre par rapport aux chaque changement apporté par Inc(i). Autrement dit, quand IncrementRate[i] égale à $BORNE_RATE[i]$, ça veut dire que la ième articulation essaie d'améliorer la solution en apportant un changement de précision 0.0001\degre.
...
\subsection{Un exemple simplifié}
Nous avons tout d'abords testé l'algorithme avec un bras manipulateur à 2 rotation. La zone atteignable de l'organe terminal est limité sur un plan dont la coordonnée de l'axe z est fixe. Alors on peut identifier un point avec seulement les coordonnées des axes x et y (Figure\ref{fig:ch2_robo2r}).

\begin{figure}[!htbp]
	\centering
		\includegraphics[scale=0.7]{pics/ch2_robo2r.jpg}
	\caption{Bras manipulateur 2R (P1=1m,P2=0.75m)}
	\label{fig:ch2_robo2r}
\end{figure}


On peut représenter cette configuration avec deux quadruplets Denavit (Figure\ref{fig:ch2_paramtable}).
\begin{figure}[!htbp]
	\centering
		\includegraphics[scale=0.9]{pics/ch2_paramtable.jpg}
	\caption{Configuration du bras 2R}
	\label{fig:ch2_paramtable}
\end{figure}

\clearpage
Selon notre implémentation, pour représenter cette configuration du bras manipulateur, il suffit de créer un tableau d'objet de classe \textbf{Quadruplet}, dont chaque objet représente un tableau dans la figure ci-dessus.
...
\begin{figure}[!htbp]
	\centering
		\includegraphics[scale=0.5]{pics/ch2_result2r.png}
	\caption{Résultat obtenu pour l'exemple 2R}
	\label{fig:ch2_result2r}
\end{figure}


L'algorithme a passé 869 itérations pour trouver la solution. Les valeurs des 2 variables articulaires finales sont affichées à la fin. Cette solution peut être interprétée comme ceci: pour atteindre le point (1.7,0,0), les 2 angles de rotation $\Theta_1,\Theta_2$ sont respectivement 12\degre,-28\degre. Il peut aussi y avoir d'autres solutions possibles. 


Si on change le point cible à (1.9,0,0), alors on va trouver que ce point n'est pas atteignable (Figure \ref{fig:ch2_result2r_nonatteignable}).


\begin{figure}[!htbp]
	\centering
		\includegraphics[scale=0.5]{pics/ch2_result2r_nonatteignable.png}
	\caption{Le résultat obtenu quand le point cible n'est pas atteignable}
	\label{fig:ch2_result2r_nonatteignable}
\end{figure}
...
\begin{figure}[!htbp]
	\centering
		\includegraphics[scale=0.4]{pics/ch2_zoneatteignable.jpg}
	\caption{Zone atteignable réelle pour l'exemple 2R}
	\label{fig:ch2_zoneatteignable}
\end{figure}

\clearpage
\subsection{Un exemple d'un bras à 5 rotation}
Nous présentons un deuxième exemple spécifique que nous avons testé, concernant un manipulateur de 5 rotations, représenté dans la figure \ref{fig:ch2_config}.
...
On crée un tableau d'objet de classe \textbf{Quadruplet}, dont chaque objet représente une transformation de repère. La dernière articulation est particulière, elle n'intervient pas dans notre travail, car la rotation de cette articulation ne change pas les coordonnées du point de l'organe terminal. Donc on a 4 objets au total dans le tableau:
...
L'algorithme a passé 27 itérations pour trouver la solution. Les valeurs des 4 variables articulaires finales sont affichées à la fin. Cette solution peut être interprétée comme ceci: pour atteindre le point (0,0,3), les 4 angles de rotation $\Theta_1,\Theta_2,\Theta_3,\Theta_4$ sont respectivement 73\degre,0\degre,90\degre,90\degre. Il peut aussi y avoir des autres solutions possibles. 